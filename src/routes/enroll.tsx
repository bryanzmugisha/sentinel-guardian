...
// Only ever called inside useEffect — safe from SSR
...
function Ring({ value, label }: { value: number; label: string }) {
  ...
  const color = value > 75 ? "#ef4444" : value > 50 ? "#eab308" : "#22c55e";
  return (
...
function EnrollPage() {
  ...
  // All browser API access is safely inside useEffect
  useEffect(() => {
    try {
  ...
  async function enroll() {
    ...
    try { localStorage.setItem("sg-device-name", deviceName); } catch {}
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-5">
      <div className="w-full max-w-sm bg-surface border border-border rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="px-6 pt-8 pb-4 flex flex-col items-center text-center">
          <div className="size-14 rounded-2xl bg-brand-accent/10 border border-brand-accent/30 flex items-center justify-center mb-4">
            <Smartphone className="size-6 text-brand-accent" />
          </div>
          <h1 className="text-lg font-bold text-white tracking-tight">Enroll this device</h1>
          <p className="text-xs text-slate-400 mt-1 leading-relaxed">
            Register this phone, tablet, or computer with Sentinel Guardian.
          </p>
        </div>
        ...
        <div className="px-6 pb-8 space-y-6">
            ...
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs mt-3 leading-relaxed">
                  Your device now appears in the Sentinel Guardian fleet table with a <strong className="text-brand-accent">live</strong> badge.
                </p>
              </div>
              <button
                onClick={enroll}
          ...
          {status === "error" && (
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <div>
                ...
                <p className="text-brand-danger text-xs mt-1 font-mono break-all">{error}</p>
              </div>
              <button
          ...
          <p className="text-[10px] text-slate-700 text-center leading-relaxed">
            Only enroll devices you own. CPU/RAM are browser estimates.
          </p>
        </div>
      </div>
    </div>
